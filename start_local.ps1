...
Start-Process -FilePath "cmd.exe" -ArgumentList "/c d:\WebGIS\start_redis.bat" -WindowStyle Minimized

# 2. MinIO Server
Write-Host "[2/5] Khoi dong MinIO Server (Port 9000/9001)..." -ForegroundColor Yellow
Start-Process -FilePath "cmd.exe" -ArgumentList "/c d:\WebGIS\start_minio.bat" -WindowStyle Minimized

# 3. Backend 1: SEGY Processing
Write-Host "[3/5] Khoi dong Backend 1: SEGY Processing (Port 8000)..." -ForegroundColor Yellow
Start-Process -FilePath "cmd.exe" -ArgumentList "/c cd /d d:\WebGIS\backend\segy-processing-service && .venv\Scripts\python.exe -m uvicorn app.main:app --host 127.0.0.1 --port 8000 --reload" -WindowStyle Minimized

# 4. Backend 2: Data Serving
Write-Host "[4/5] Khoi dong Backend 2: Data Serving (Port 8001)..." -ForegroundColor Yellow
Start-Process -FilePath "cmd.exe" -ArgumentList "/c cd /d d:\WebGIS\backend\data-serving && .venv\Scripts\python.exe -m uvicorn app.main:app --host 127.0.0.1 --port 8001 --reload" -WindowStyle Minimized

# 5. Frontend
Write-Host "[5/5] Khoi dong Frontend (Port 5173)..." -ForegroundColor Yellow
Start-Process -FilePath "cmd.exe" -ArgumentList "/c cd /d d:\WebGIS\frontend && npm run dev" -WindowStyle Minimized
